defmodule Vault.Commands.Restore do
      ...
      case File.ls(source_home) do
        {:ok, entries} ->
          ...
          |> Enum.filter(fn entry ->
            entry != "Library" and not String.starts_with?(entry, ".")
          end)
          |> Enum.each(fn entry ->
  ...
  defp restore_dotfiles_and_local_bin(vault_path, home_dir, dry_run) do
    ...
    if File.dir?(dotfiles_src) do
      case File.ls(dotfiles_src) do
        {:ok, items} ->
          items
          |> Enum.reject(fn name ->
            name == ".config" or name == "mise.toml" or name == ".zsh_history"
          end)
          |> Enum.each(fn name ->
    ...
    if File.exists?(zsh_history_src) do
      if dry_run do
        Progress.puts([
          ...
          " would restore .zsh_history"
        ])
      else
        Vault.Sync.copy_tree(zsh_history_src, zsh_history_dest)
    ...
    if File.exists?(mise_toml_src) do
      if dry_run do
        ...
        Vault.Sync.copy_tree(mise_toml_src, mise_toml_dest)
  ...
  defp restore_app_support(vault_path, home_dir, dry_run) do
    ...
    if File.dir?(src) do
      ...
      Progress.puts([
        ...
        "No Application Support data found in vault/app-support/"
      ])
    end
  end
        ...
        case File.ls(src) do
          {:ok, files} ->
            ...
            Enum.each(files, fn plist ->
              ...
              Vault.Sync.copy_tree(src_file, dest_file)
            end)

            Progress.puts([
              "  ",
              Progress.tag("✓", :green),
              " Restored #{length(files)} preference files"
            ])
  ...
  defp restore_sensitive(vault_path, home_dir, dry_run) do
    ...
    if not File.dir?(sensitive_src) do
      Progress.puts([
        Progress.tag("  ℹ ", :yellow),
        "No sensitive data found in vault/sensitive/"
      ])
    else
            ...
            case dir_name do
              "ssh" ->
                ...
                Progress.puts([
                  "  ",
                  Progress.tag("✓", :green),
                  " Restored SSH keys with secure permissions"
                ])
              ...
              "private" ->
                Progress.puts([
                  "  ",
                  Progress.tag("✓", :green),
                  " Restored passwords from .config/private"
                ])
  ...
  defp restore_brave(vault_path, home_dir, dry_run) do
    ...
    if File.dir?(src) do
      Progress.puts([
        "  ",
        Progress.tag("ℹ ", :yellow),
        "Ensure Brave is closed before restoring."
      ])
      ...
      Progress.puts([
        "  ",
        Progress.tag("ℹ ", :yellow),
        "Passwords/cookies are Keychain-bound and may not transfer. Use Brave Sync or password export/import."
      ])
    else
      Progress.puts([
        Progress.tag("  ℹ ", :yellow),
        "No Brave data found in vault/browser/brave/"
      ])
    end
  end
  ...
  defp restore_obsidian(vault_path, dest_base, dry_run) do
    ...
    if File.dir?(src_base) do
      Progress.puts([
        "  ",
        Progress.tag("ℹ ", :yellow),
        "Ensure Obsidian is closed before restoring."
      ])
